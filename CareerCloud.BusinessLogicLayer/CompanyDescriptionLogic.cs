   ...
   public class CompanyDescriptionLogic : BaseLogic<CompanyDescriptionPoco>
    {
        ...
        protected override void Verify(CompanyDescriptionPoco[] pocos)
        {
            ...
            foreach(CompanyDescriptionPoco poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException(107, "Wrong CompanyDescription Name"));
                }
                if (string.IsNullOrEmpty(poco.CompanyName))
                {
                    exceptions.Add(new ValidationException(106, "Wrong CompanyName"));
                }
                else if (poco.CompanyName.Length < 3)
                {
                    exceptions.Add(new ValidationException(106, "CompanyName can't be less than two char"));

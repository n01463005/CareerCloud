   ...
   public class CompanyLocationLogic : BaseLogic<CompanyLocationPoco>
    {
        ...
        protected override void Verify(CompanyLocationPoco[] pocos)
        {
            ...
            foreach(CompanyLocationPoco poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException(500, "wrong country code"));
                }
                if (string.IsNullOrEmpty(poco.Province))
                {
                    exceptions.Add(new ValidationException(501, "Wrong Province"));
                }
                if (string.IsNullOrEmpty(poco.Street))
                {
                    exceptions.Add(new ValidationException(502, "Wrong street"));
                }
                if (string.IsNullOrEmpty(poco.City))
                {
                    exceptions.Add(new ValidationException(503, "wrong city"));
                }
                if (string.IsNullOrEmpty(poco.PostalCode))
                {
                    exceptions.Add(new ValidationException(504, " empty postalCode"));
                }
